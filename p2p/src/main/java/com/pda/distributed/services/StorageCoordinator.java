...
public class StorageCoordinator {
	...
	public void setNetworkService(NetworkService networkService) { this.networkService = networkService; }
	public void setQuorumService(QuorumService quorumService) { this.quorumService = quorumService; }
	...
	while (activo) {
            try {
                ...
                // Usamos Math.ceil para redondear hacia arriba (ej. 5MB / 2MB = 2.5 -> 3 pedazos)
                ...
                DistributedDirectory.FileMetadata metadata = new DistributedDirectory.FileMetadata(nombreArchivo, fileBytes.length);
                ...
                ConsoleLogger.info("Coordinator", "Partiendo " + nombreArchivo + " en " + totalChunks + " fragmentos...");
                ...
                // TODO (Futuro): Avisarle al StateSyncService que haga un broadcast del mapa actualizado
	...
	public List<String> allocateToNodes(DistributedDirectory.FileMetadata fileMetadata) {
        // En una implementación real, le preguntaríamos al QuorumService qué nodos del RING_B 
        // tienen más espacio. Por ahora, tomaremos las conexiones activas en el NetworkService.
    ...
    */
    public void handleIncomingChunk(String fileId, int chunkIndex, byte[] data, int totalExpected) {
        ...
        // Le pasamos la papa caliente al StorageManager para que lo guarde en el disco local
